function [W_Csp] = CSP_training_Real_Data_Downsampling(b_fir_mu)
...
for k = 1:1:20
    load(['/Users/matthieu/GitHub/BMI_MasterThesis/GUI/RealDataMat/26_03_18_Right'  num2str(k) '.mat']);
    C3 = downsample(C3,2);
    C4 = downsample(C4,2);
    Cz = downsample(Cz,2);
    ...
    R_normalized_current = spatial_cov_computation_Real_Data(C3_filtered,C4_filtered,Cz_filtered);
...
for k = 1:1:20
    load(['/Users/matthieu/GitHub/BMI_MasterThesis/GUI/RealDataMat/26_03_18_Left'  num2str(k) '.mat']);
    C3 = downsample(C3,2);
    C4 = downsample(C4,2);
    Cz = downsample(Cz,2);
    C3_filtered = filter(b_fir_mu,1,C3); % C3_prime but let's modify it
...
R_ave_left = R_ave_left./10; %%%% ATTENTION BY 20 HOPEFULLY ?
